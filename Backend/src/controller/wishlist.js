...
exports.addItemToWishList = (req, res) => {
  WishList.findOne({ user: req.user._id }).exec((error, cart) => {
...
//       if(error) return res.status(400).json({ error });
//       if(cart){
//           //if already cart then update quantity

//           let promiseArray = []; 
//           req.body.cartItems.findEach((cartItem) =>{})
//         const product = cartItem.product;  
//         const item =  cart.cartItems.find(c => c.product == product );
//         let condition, update;
...
//            if(error) return res.status(400).json({ error });
//            if(_cart){
//                return res.status(400).json({ cart: _cart });
//            }
//            })

//       }else{
//           // if cart not then create
//           const cart = new Cart({
//               user: req.user._id,
//               cartItems: req.body.cartItems
//           });
